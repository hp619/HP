import random
import string
import requests # Naya: Address to coords conversion ke liye
...
# --- Naya Helper Function: Address se Coordinates nikalne ke liye ---
def get_coordinates(address):
    from app import GOOGLE_MAPS_API_KEY
    if not address:
        return None
    # Address ko URL safe banane ke liye aur sahi response ke liye
    url = f"https://maps.googleapis.com/maps/api/geocode/json?address={address}&key={GOOGLE_MAPS_API_KEY}"
    try:
        response = requests.get(url).json()
        if response['status'] == 'OK':
            loc = response['results'][0]['geometry']['location']
            # FIXED: MongoDB [lng, lat] format leta hai
...
def login():
    if request.method == 'POST':
        ...
        if user:
            ...
            # Additional session data for SOS logic
...
def verify_reset_password():
    ...
    if str(user_otp) != str(session.get('reset_otp')):
        flash("OTP sahi nahi hai! Dobara check karein.", "danger")
        return render_template('reset_password.html')

    if new_password != conf_password:
        flash("Dono Passwords match nahi kar rahe!", "danger")
        return render_template('reset_password.html')
    ...
    flash("Password badal diya gaya hai! Ab login karein.", "success")
    return redirect(url_for('auth_bp.login'))
...
def verify_and_register():
    ...
    if str(user_otp) == str(session.get('otp')):
        ...
        # --- FIXED: Location Logic with Float conversion ---
        lat = reg_data.get('lat')
        lng = reg_data.get('lng')
        location_obj = None
        
        if lat and lng:
            # MongoDB strict order: [Longitude, Latitude]
        ...
        # Role based mapping
        if role == 'patient':
            reg_data['patient_name'] = reg_data.get('patient_name') or reg_data.get('Full Name') or reg_data.get('name')
            reg_data['aadhar_no'] = reg_data.get('aadhar_no') or reg_data.get('Aadhar Number (12 Digits)')
            reg_data['phone'] = reg_data.get('phone') or ""
            reg_data['location'] = location_obj # Patient location save karein
        else:
            # Hospital logic
            reg_data['hospital_name'] = reg_data.get('hospital_name') or reg_data.get('h-name') or reg_data.get('Hospital Name')
            reg_data['phone'] = reg_data.get('phone') or ""
            reg_data['type'] = 'physical'
            reg_data['location'] = location_obj

        # 1. 'users' collection me insert
        db.users.insert_one(reg_data)
        
        # 2. 'hospitals' collection me sync karein
        if role == 'hospital':
        ...
        flash(f"Registration Successful! Aapka Login ID: {reg_id}", "success")
        return jsonify({"status": "success"}), 200
